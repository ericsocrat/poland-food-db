...
│   │   ├── QA__scoring_formula_tests.sql  # 25 scoring validation checks
...
| `ingredients`     | Raw ingredient text + additive count      | `product_id`                | `additives_count` feeds scoring; `ingredients_raw` populated from OFF `ingredients_text` when available |
