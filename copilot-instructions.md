...
> **QA:** 51 critical checks + 8 informational reports — all passing
...
│   ├── qa/                          # Test suites
│   │   ├── QA__null_checks.sql      # 22 data integrity checks
...
├── supabase/
│   ├── config.toml
│   └── migrations/                  # 21 append-only schema migrations
...
| `ingredients`     | Raw ingredient text + additive count      | `product_id`                | `additives_count` feeds scoring; `ingredients_raw` populated from OFF `ingredients_text` (493/560 = 88%) |
...
**`v_master`** — Flat denormalized join: products → servings → nutrition_facts → scores → ingredients → sources (via `category` equijoin). Filtered to `is_deprecated = false`. This is the primary query surface.
