...
| `servings`           | Serving definitions (per 100g + per serving) | `serving_id` (identity)                 | FK → products; 877 per-100g rows                                                                      |
...
| `product_sources`    | Product-level data provenance                | `product_source_id` (identity)          | 877 rows; FK → products; confidence_pct, is_primary                                               |
