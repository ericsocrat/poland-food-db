...
> **QA:** 144 critical checks + 14 informational reports — all passing
...
│   │   ├── QA__confidence_scoring.sql  # 10 confidence scoring checks│   │   ├── QA__data_quality.sql          # 28 data quality checks
│   │   ├── QA__referential_integrity.sql # 17 referential integrity checks
│   │   ├── QA__view_consistency.sql      # 10 view consistency checks│   │   └── QA__source_coverage.sql  # 8 informational reports (non-blocking)
│   └── views/
...
├── RUN_QA.ps1                       # QA test runner (153 critical + 8 info)
...
| `ingredient_ref`     | Canonical ingredient dictionary              | `ingredient_id` (identity)              | 1,164 unique ingredients; name_en, vegan/vegetarian/palm_oil/is_additive/concern_tier flags       |
| `product_ingredient` | Product ↔ ingredient junction                | `(product_id, ingredient_id, position)` | ~7,403 rows; tracks percent, percent_estimate, sub-ingredients, position order                    |
...
| `product_sources`    | Product-level data provenance                | `product_source_id` (identity)          | 560 rows (off_api only); FK → products; confidence_pct, is_primary                                |
...
**`v_master`** — Flat denormalized join: products → servings → nutrition_facts → scores → product_sources (via LATERAL, primary row) + ingredient analytics via LATERAL subqueries on product_ingredient + ingredient_ref (ingredient_count, additive_names, ingredients_raw, has_palm_oil, vegan_status, vegetarian_status, allergen_count/tags, trace_count/tags). Includes `score_breakdown` (JSONB), `ingredient_data_quality`, and `nutrition_data_quality` columns. 63 columns total. Filtered to `is_deprecated = false`. This is the primary internal query surface.
...
**Order matters:** Products (01) must exist before servings (02) and nutrition (03). Scoring (04) creates score rows if missing, then computes all values.
...
unhealthiness_score = compute_unhealthiness_v32(
    ...
    nf.calories::numeric, nf.trans_fat_g::numeric, ia.additives_count::numeric,
...
| child tables | FK PK indexes | `product_id` (scores, servings, etc.) |
...
| ---------------- | ------------------------------- | -----: | --------- |
| Data Integrity   | `QA__null_checks.sql`           |     35 | Yes       |
...
| Data Quality     | `QA__data_quality.sql`          |     28 | Yes       |
| Ref. Integrity   | `QA__referential_integrity.sql` |     17 | Yes       |
| View Consistency | `QA__view_consistency.sql`      |     10 | Yes       |

**Run:** `.\RUN_QA.ps1` — expects **144/144 critical checks passing**.
