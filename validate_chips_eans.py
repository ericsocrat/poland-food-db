...
print("=" * 65)
print(
    f"Results: {len(valid_eans)} valid EAN-13, {len(invalid_eans)} invalid/non-standard"
)
...
if valid_eans:
    ...
    for item in valid_eans:
        ...
        print(
            f"UPDATE products SET ean = '{item['ean']}' WHERE brand = '{brand_sql}' AND product_name = '{product_name_sql}' AND category = 'Chips';"
        )
...
if invalid_eans:
    ...
    for item in invalid_eans:
        print(
            f"  - {item['brand']} {item['product_name']}: {item['ean']} ({len(item['ean'])} digits)"
        )
