...
3. First active country in `country_ref` (alphabetical)

This guarantees a non-NULL country scope — **no mixed-country results are ever returned**.
...
**Description:** Returns the authenticated user's preference profile. If no preferences have been set, returns a message prompting configuration.
...
**Description:** Creates or updates (upserts) the authenticated user's preference profile. Validates country against `country_ref` and diet preference enum. Returns the updated profile (via `api_get_user_preferences()`).
...
| Property | Details |
|----------|---------|
| `p_country` default | `'PL'` (hardcoded, not auto-resolved) |
| Uses `resolve_effective_country()` | **No** |
| Auth required | **Yes** (`auth.uid()`) |
...
|----------|-------------------|---------|-------------------------------|---------------|-------|
| `api_search_products` | Yes | `NULL` | **Yes** | No | anon, auth, service |
| `api_category_listing` | Yes | `NULL` | **Yes** | No | anon, auth, service |
| `api_product_detail` | No | — | No | No | anon, auth, service |
| `api_product_detail_by_ean` | Yes | `NULL` | **Yes** | No | anon, auth, service |
| `api_better_alternatives` | No | — | No (inferred from product) | No | anon, auth, service |
| `api_score_explanation` | No | — | No | No | anon, auth, service |
| `api_data_confidence` | No | — | No | No | anon, auth, service |
| `api_get_user_preferences` | No | — | No | **Yes** | auth, service |
| `api_set_user_preferences` | Yes | `'PL'` | No | **Yes** | auth, service |
