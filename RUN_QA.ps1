...
Write-Host ""
Write-Host "Running Test Suite 2: Scoring Formula (15 checks)..." -ForegroundColor Yellow
...
if ($test2Lines -eq "" -or $test2Lines -match '^\s*$') {
    Write-Host "  ✓ PASS (15/15 — zero violations)" -ForegroundColor Green
    $test2Pass = $true
}
else {
    Write-Host "  ✗ FAILED — violations detected:" -ForegroundColor Red
...
if ($test1Pass -and $test2Pass) {
    Write-Host "  ✓ ALL TESTS PASSED (26/26 checks)" -ForegroundColor Green
    Write-Host ""
    exit 0
}
else {
