...
for brand, product_name, ean in bread_eans:
    ...
    if is_valid:
        ...
        print(f"✗ {brand:20s} {product_name:50s} {ean} (expected: {expected}, got: {actual})")
        invalid_count += 1
...
if invalid_eans:
    ...
    print(f"✓ Ready to apply with: docker exec supabase_db_poland-food-db psql -U postgres -d postgres -f /docker-entrypoint-initdb.d/migrations/20260208_add_bread_eans.sql")
