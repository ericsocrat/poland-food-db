-- ─── Fix: Deduplicate top_ingredients in api_get_product_profile() ────────────
-- The ingredient_ref table contains case-variant duplicates (e.g. "salt" and
-- "Salt") because OFF data was imported with mixed casing.  The top_ingredients
-- subquery now collapses duplicates using DISTINCT ON (LOWER(ir.name_en)),
-- keeping the row with the smallest position for each name.
-- ─────────────────────────────────────────────────────────────────────────────
