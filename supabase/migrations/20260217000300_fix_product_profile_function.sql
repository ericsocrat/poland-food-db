-- ─── Fix: Restore api_get_product_profile() ─────────────────────────────────
-- Migration 20260217000100 (ingredient_profile_api) re-created
-- api_get_product_profile() but introduced column-reference bugs:
--   • FROM products m  → should be FROM v_master m
--   • cat.name_translations, cat.name_en  → columns don't exist on category_ref
--   • cat.icon  → should be cr.icon_emoji
--   • Wrong nutrition column names (energy_kcal → calories, fat_g → total_fat_g)
--   • Missing category_translations / nutri_score_ref joins
--   • Missing score_breakdown, category_context, compute_data_confidence, etc.
--
-- This migration restores the full correct body from 20260216001500
-- (product_images) and keeps the ingredient_id + concern_reason additions
-- that 20260217000100 intended.
-- ─────────────────────────────────────────────────────────────────────────────
...
'ingredients (with ingredient_id for linking), allergens, scores with '
'breakdown + category context, warnings, quality/confidence, top 3 '
'alternatives, flags, and product images in a single JSONB envelope.';
