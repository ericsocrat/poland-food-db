...
-- ─────────────────────────────────────────────────────────────────────────────
-- #1  api_product_detail — top-level keys (18)
-- ─────────────────────────────────────────────────────────────────────────────
SELECT
    CASE WHEN (
        ...
        'country','ean','flags','ingredients','nutrition_per_100g','prep_method',
        'product_id','product_name','product_type','scores','store_availability','trust'
    ]
    THEN 'PASS' ELSE 'FAIL' END AS "#1  product_detail top-level keys (18)";
...
-- ─────────────────────────────────────────────────────────────────────────────
-- #8  api_search_products — top-level keys (7)
-- ─────────────────────────────────────────────────────────────────────────────
SELECT
    CASE WHEN (
    ...
    THEN 'PASS' ELSE 'FAIL' END AS "#8  search_products top-level keys (7)";

-- ─────────────────────────────────────────────────────────────────────────────
-- #9  api_search_products → result item keys (9)
-- ─────────────────────────────────────────────────────────────────────────────
SELECT
    CASE WHEN (
    ...
    THEN 'PASS' ELSE 'FAIL' END AS "#9  search_products → item keys (9)";

-- ─────────────────────────────────────────────────────────────────────────────
-- #10 api_category_listing — top-level keys (8)
-- ─────────────────────────────────────────────────────────────────────────────
SELECT
    CASE WHEN (
    ...
    THEN 'PASS' ELSE 'FAIL' END AS "#10 category_listing top-level keys (8)";

-- ─────────────────────────────────────────────────────────────────────────────
-- #11 api_category_listing → product item keys (19)
-- ─────────────────────────────────────────────────────────────────────────────
SELECT
    CASE WHEN (
    ...
    THEN 'PASS' ELSE 'FAIL' END AS "#11 category_listing → item keys (19)";

-- ─────────────────────────────────────────────────────────────────────────────
-- #12 api_score_explanation — top-level keys (10)
-- ─────────────────────────────────────────────────────────────────────────────
SELECT
    CASE WHEN (
    ...
    THEN 'PASS' ELSE 'FAIL' END AS "#12 score_explanation top-level keys (10)";

-- ─────────────────────────────────────────────────────────────────────────────
-- #13 api_score_explanation → summary keys (6)
-- ─────────────────────────────────────────────────────────────────────────────
SELECT
    CASE WHEN (
    ...
    THEN 'PASS' ELSE 'FAIL' END AS "#13 score_explanation → summary keys (6)";

-- ─────────────────────────────────────────────────────────────────────────────
-- #14 api_score_explanation → category_context keys (4)
-- ─────────────────────────────────────────────────────────────────────────────
SELECT
    CASE WHEN (
    ...
    THEN 'PASS' ELSE 'FAIL' END AS "#14 score_explanation → category_context keys (4)";

-- ─────────────────────────────────────────────────────────────────────────────
-- #15 api_better_alternatives — top-level keys (5)
-- ─────────────────────────────────────────────────────────────────────────────
SELECT
    CASE WHEN (
    ...
    THEN 'PASS' ELSE 'FAIL' END AS "#15 better_alternatives top-level keys (5)";

-- ─────────────────────────────────────────────────────────────────────────────
-- #16 api_better_alternatives → source_product keys (6)
-- ─────────────────────────────────────────────────────────────────────────────
SELECT
    CASE WHEN (
    ...
    THEN 'PASS' ELSE 'FAIL' END AS "#16 alternatives → source_product keys (6)";

-- ─────────────────────────────────────────────────────────────────────────────
-- #17 api_data_confidence — top-level keys (8)
-- ─────────────────────────────────────────────────────────────────────────────
SELECT
    CASE WHEN (
    ...
    THEN 'PASS' ELSE 'FAIL' END AS "#17 data_confidence top-level keys (8)";

-- ─────────────────────────────────────────────────────────────────────────────
-- #18 api_data_confidence → components keys (5)
-- ─────────────────────────────────────────────────────────────────────────────
SELECT
    CASE WHEN (
    ...
    THEN 'PASS' ELSE 'FAIL' END AS "#18 data_confidence → components keys (5)";

-- ─────────────────────────────────────────────────────────────────────────────
-- #19 api_data_confidence → data_completeness_profile keys (3)
-- ─────────────────────────────────────────────────────────────────────────────
SELECT
    CASE WHEN (
    ...
    THEN 'PASS' ELSE 'FAIL' END AS "#19 confidence → completeness_profile keys (3)";

-- ─────────────────────────────────────────────────────────────────────────────
-- #20 All api_* functions have api_version = '1.0'
-- ─────────────────────────────────────────────────────────────────────────────
SELECT
    CASE WHEN (
        SELECT COUNT(*) = 6 FROM (
    ...
    THEN 'PASS' ELSE 'FAIL' END AS "#20 all API functions return api_version = 1.0";

-- ─────────────────────────────────────────────────────────────────────────────
-- #21 api_search_products error path includes api_version
-- ─────────────────────────────────────────────────────────────────────────────
SELECT
    CASE WHEN api_search_products('x')->>'api_version' = '1.0'
         AND api_search_products('x') ? 'error'
    THEN 'PASS' ELSE 'FAIL' END AS "#21 search error response includes api_version";

-- ─────────────────────────────────────────────────────────────────────────────
-- #22 All api_* functions are SECURITY DEFINER (still, after recreation)
-- ─────────────────────────────────────────────────────────────────────────────
SELECT
    CASE WHEN COUNT(*) = 6
    THEN 'PASS' ELSE 'FAIL' END AS "#22 all api_* remain SECURITY DEFINER"
FROM pg_proc p
